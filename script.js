...
// LaTeX equation formatting - Correction of excessive $$
...
        .replace(/([a-zA-Z])([A-Z])/g, '$1 $2') // Add space between glued words
...
        if (!drift || !diffusion) {
            throw new Error('Please enter the drift and diffusion coefficients');
        }
...
            } catch (e) {
                throw new Error('Invalid JSON in the parameters field');
            }
        }
...
            throw new Error('The final time must be greater than the initial time');
        }
...
    });
    
    // Layout with Palatino font and custom style
    const layout = {
        font: {
...
        a.href = url;
        a.download = `solution_sde_${Date.now()}.pdf`;
...
        if (!response.ok) {
            throw new Error('LaTeX file generation failed');
        }
...
        a.style.display = 'none';
        a.href = url;
        a.download = `solution_sde_${Date.now()}.tex`;
        document.body.appendChild(a);
        a.click();
        window.URL.revokeObjectURL(url);
        document.body.removeChild(a);
...
        
        // Update fields according to process type
...
        
        // Update examples in the interface
